...
namespace FileManagmentApp.Model
{
    internal class DLList<T>
    {
        ...
        public void Traverse(double totalCount)
        {
            ...
            {
                Console.WriteLine($"Item: {node.Value}, Count: {node.Count}, " + $"Frequency: {(node.Count / totalCount):P}");
                node = node.Next;
            }
        }
        ...
        {
            for (GenericNode<T>? iNode = Head; iNode is not null; iNode = iNode.Next)
            {
                T? minVal = iNode.Value;
                GenericNode<T>? minPos = iNode;

                for (GenericNode<T>? jNode = iNode; jNode is not null; jNode = jNode.Next)
                {
